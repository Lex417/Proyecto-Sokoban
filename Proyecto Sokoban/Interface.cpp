...
Interface::Interface() : VentanaPrincipal{ VideoMode{ 900,700 },"Sokoban.exe",Style::Titlebar | Style::Close }
{
	completo1 = false;
	completo2 = false;
	completo3 = false;
...
void Interface::deleteReplay()
{
	int i = 0;
	while (i < static_cast<int>(repetition.size()))
	{
		repetition[i].pop_back();
		i++;
	}
	repetition.resize(0);
}
...
void Interface::paintLevel(int x, List list, int num)
{
...
int Interface::initializeWindow()
{
	...
	if (!wall.loadFromFile("Sprites/wall.jpg")) 
	{
		cout << "No se ha encontrado la imagen&pause";
		return -1;
	}
	wallsSprite.setTexture(wall);
	if (!emptySpace.loadFromFile("Sprites/GroundGravel_Concrete.png")) 
	{
	...
	if (!finish.loadFromFile("Sprites/Finish.jpg"))
	{
		system("No se ha encontrado la imagen&pause");
		return -1;
	}
	winner.setTexture(finish);

	if (!box.loadFromFile("Sprites/CrateDark_Red.png")) 
	{
		system("No se ha encontrado la imagen&pause");
		return -1;
	}
	boxesSprite.setTexture(box);
	if (!playerDown.loadFromFile("Sprites/Character4.png"))
	{
		system("No se ha encontrado la imagen&pause");
		return -1;
	}
	playerSprite.setTexture(playerDown);
	if (!playerUp.loadFromFile("Sprites/CharacterArriba.png")) 
	{
		system("No se ha encontrado la imagen&pause");
		return -1;
	}
	if (!playerRight.loadFromFile("Sprites/CharacterDer.png")) 
	{
		system("No se ha encontrado la imagen&pause");
		return -1;
	}
	if (!playerLeft.loadFromFile("Sprites/CharacterIzq.png"))
	{
		system("No se ha encontrado la imagen&pause");
		return -1;
	}
	if (!winnigPoints.loadFromFile("Sprites/endpoint.png")) 
	{
		system("No se ha encontrado la imagen&pause");
		return -1;
	}
	...
	if (!red.loadFromFile("Sprites/Red_skin.png"))
	{
		system("No se ha encontrado la imagen&pause");
		return -1;
	}
...
void Interface::nextLevel(bool done)
{
	if (done) 
...
void Interface::movements()
{
	...
	if (Keyboard::isKeyPressed(Keyboard::LShift)) // restart actual level
	{
		deleteReplay();
		deleteLists();
		system("cls");
	...
	if (Keyboard::isKeyPressed(Keyboard::BackSpace)) { // clean all the list and initiate the menu again
		deleteReplay();
		deleteLists();
		completo1 = false;
		completo2 = false;
...
void Interface::run()
{
	...
	{
		system("No se ha encontrado la cancion&pause");
		return;
	}
	music.play();
	initializeWindow();
...
void Interface::fillLists()
{
	...
	{								//SI EL NIVEL 1 EST� COMPLETO  Y EL NIVEL 2 N0..., SE REALIZA EL MISMO RECORRIDO 
	...
	{						//SI EL NIVEL 1 Y 2 EST�N COMPLETOS  Y EL NIVEL 3 N0..., SE REALIZA EL MISMO RECORRIDO 
...
void Interface::setCompleto1() {
	completo1 = true;
}

void Interface::setCompleto2() {
	completo2 = true;
}

void Interface::setCompleto3() {
	completo3 = true;
}

void Interface::setCompleto4() {
	completo4 = true;
}

void Interface::setCompleto5() {
	completo5 = true;
}

void Interface::initiateMenu() {
